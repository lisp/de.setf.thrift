      ...
      (if fun
        ...
        (dolist (base-service (service-base-services service))
          (multiple-value-bind (fun service)
                               (find-thrift-function identifier base-service)
            (when fun (return-from find-thrift-function (values fun service)))))))))
  ...
  (:method ((service service) (protocol t))
    (flet ((consume-message ()
      ...
      (multiple-value-bind (identifier type sequence-number) (stream-read-message-begin protocol)
        (ecase type
          ((call oneway)
           (multiple-value-bind (operator service)
                                (find-thrift-function service identifier)
             (cond (operator
