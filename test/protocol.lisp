...
;;; (run-tests "vector-transport.*")

(test vector-transport.open-stream-p
...
(test vector-transport.stream-read/write-double
  (let ((stream (make-test-protocol)))
    (every #'(lambda (entry)
               (apply #'test-read-write-equivalence stream entry))
...
(test vector-transport.stream-read/write-string
  (let ((stream (make-test-protocol)))
    (every #'(lambda (entry)
               (apply #'test-read-write-equivalence stream entry))
           `((stream-read-string stream-write-string "a" "0123456789" "0.999Û")))))


(test vector-transport.stream-read/write-binary
  (let ((stream (make-test-protocol)))
    (every #'(lambda (entry)
               (apply #'test-read-write-equivalence stream entry))
...
(test vector-transport.stream-read/write-struct
  (let ((struct (make-instance 'test-struct :field1 "one" :field2 2))
        (stream (make-test-protocol)))
...
(test vector-transport.stream-read/write-field
  (let ((stream (make-test-protocol)))
    (every #'(lambda (entry)
               (apply #'test-read-write-equivalence stream entry))
           `((,(lambda (p) (multiple-value-bind (value name id)
                                                (stream-read-field p)
                             (when (ecase (thrift::protocol-field-key stream)
                                     (:name (and (equal name "test") (null id)))
                                     (:identifier (and (null name) (equal id 10))))
                               value)))
              ,(lambda (p v) (stream-write-field p v :name "test" :id 10))
              "a" "0123456789" "0.999Û")))))


(test vector-transport.stream-read/write-map
  (let ((stream (make-test-protocol)))
    (every #'(lambda (entry)
               (apply #'test-read-write-equivalence stream entry))
...
(test vector-transport.stream-read/write-list
  (let ((stream (make-test-protocol)))
    (every #'(lambda (entry)
               (apply #'test-read-write-equivalence stream entry))
...
(test vector-transport.stream-read/write-set
  (let ((stream (make-test-protocol)))
    (every #'(lambda (entry)
               (apply #'test-read-write-equivalence stream entry))
